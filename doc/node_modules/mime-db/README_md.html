...
<div class="nav-section">
  ...
  <ul class="link-list" role="directory">
    <li><a href="#label-mime-db">mime-db</a>
...
<p><a href="https://npmjs.org/package/mime-db"><img
src="https://badgen.net/npm/v/mime-db"></a> <a
href="https://npmjs.org/package/mime-db"><img
src="https://badgen.net/npm/dm/mime-db"></a> <a
href="https://nodejs.org/en/download"><img
src="https://badgen.net/npm/node/mime-db"></a> <a
href="https://travis-ci.org/jshttp/mime-db"><img
src="https://badgen.net/travis/jshttp/mime-db/master"></a> <a
href="https://coveralls.io/r/jshttp/mime-db?branch=master"><img
src="https://badgen.net/coveralls/c/github/jshttp/mime-db/master"></a></p>

<p>This is a database of all mime types. It consists of a single, public JSON
file and does not include any logic, allowing it to remain as
un-opinionated as possible with an API. It aggregates data from the
following sources:</p>
<ul><li>
<p><a
href="http://www.iana.org/assignments/media-types/media-types.xhtml">www.iana.org/assignments/media-types/media-types.xhtml</a></p>
</li><li>
<p><a
href="http://svn.apache.org/repos/asf/httpd/httpd/trunk/docs/conf/mime.types">svn.apache.org/repos/asf/httpd/httpd/trunk/docs/conf/mime.types</a></p>
</li><li>
<p><a
href="http://hg.nginx.org/nginx/raw-file/default/conf/mime.types">hg.nginx.org/nginx/raw-file/default/conf/mime.types</a></p>
</li></ul>
...
<p>If you&#39;re crazy enough to use this in the browser, you can just grab
the JSON file using <a href="https://www.jsdelivr.com/">jsDelivr</a>. It is
recommended to replace <code>master</code> with <a
href="https://github.com/jshttp/mime-db/tags">a release tag</a> as the JSON
format may change in the future.</p>
...
<p>The JSON file is a map lookup for lowercased mime types. Each mime type has
the following properties:</p>
<ul><li>
<p><code>.source</code> - where the mime type is defined. If not set, it&#39;s
probably a custom media type.</p>
<ul><li>
<p><code>apache</code> - <a
href="http://svn.apache.org/repos/asf/httpd/httpd/trunk/docs/conf/mime.types">Apache
common media types</a></p>
</li><li>
<p><code>iana</code> - <a
href="http://www.iana.org/assignments/media-types/media-types.xhtml">IANA-defined
media types</a></p>
</li><li>
<p><code>nginx</code> - <a
href="http://hg.nginx.org/nginx/raw-file/default/conf/mime.types">nginx
media types</a></p>
</li></ul>
</li><li>
<p><code>.extensions[]</code> - known extensions associated with this mime
type.</p>
</li><li>
<p><code>.compressible</code> - whether a file of this type can be gzipped.</p>
</li><li>
<p><code>.charset</code> - the default charset associated with this type, if
any.</p>
</li></ul>
...
<p>The <code>src/custom.json</code> file is a JSON object with the MIME type
as the keys and the values being an object with the following keys:</p>
<ul><li>
<p><code>compressible</code> - leave out if you don&#39;t know, otherwise
<code>true</code>/<code>false</code> to  indicate whether the data
represented by the type is typically compressible.</p>
</li><li>
<p><code>extensions</code> - include an array of file extensions that are
associated with  the type.</p>
</li><li>
<p><code>notes</code> - human-readable notes about the type, typically what
the type is.</p>
</li><li>
<p><code>sources</code> - include an array of URLs of where the MIME type and
the associated  extensions are sourced from. This needs to be a <a
href="https://en.wikipedia.org/wiki/Primary_source">primary source</a>; 
links to type aggregating sites and Wikipedia are <em>not acceptable</em>.</p>
</li></ul>

<p>To update the build, run <code>npm run build</code>.</p>

<h2 id="label-Adding+Custom+Media+Types">Adding Custom Media Types<span><a href="#label-Adding+Custom+Media+Types">&para;</a> <a href="#top">&uarr;</a></span></h2>

<p>The best way to get new media types included in this library is to register
them with the IANA. The community registration procedure is outlined in <a
href="http://tools.ietf.org/html/rfc6838#section-5">RFC 6838 section 5</a>.
Types registered with the IANA are automatically pulled into this library.</p>
</main>
