...
<main role="main" aria-label="Page node_modules/bytes/index.js">
...
<p>module.exports = bytes; module.exports.format = format;
module.exports.parse = parse;</p>
...
<p>var formatThousandsRegExp = /B(?=(d{3})+(?!d))/g;</p>

<p>var formatDecimalsRegExp = /(?:.<a href="^0">0*|(.</a>+)0+)$/;</p>
...
gb: 1 &lt;&lt; 30,
tb: Math.pow(1024, 4),
pb: Math.pow(1024, 5),</pre>

<p>};</p>

<p>var parseRegExp = /^((-|+)?(d+(?:.d+)?)) *(kb|mb|gb|tb|pb)$/i;</p>

<p>/**</p>

<pre> Convert the given value in bytes into a string or parse to string to an integer in bytes.
...
<p>function bytes(value, options) {</p>
...
<pre> Format the given value in bytes into a string.
...
<p>function format(value, options) {</p>

<pre>if (!Number.isFinite(value)) {
  return null;
}
...
return str + unitSeparator + unit;</pre>

<p>}</p>

<p>/**</p>

<pre> Parse the string value into an integer in bytes.
...
<p>function parse(val) {</p>

<pre>if (typeof val === &#39;number&#39; &amp;&amp; !isNaN(val)) {
  return val;
}
...
return Math.floor(map[unit] * floatValue);</pre>
